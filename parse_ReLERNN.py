...
for p in populations:
    ...
    path = "/home/prm/Skrivbord/ReLERNN_plotting/results2/"

    rmap = pd.read_csv(path+p+"_biallelic.PREDICT.txt", sep = "\t")
    rmap['chrom'] = rmap['chrom'].str.replace("'", "").str.replace("b", "")
    rmap = rmap.dropna()
       
    rmap['L'] = rmap['end'] - rmap['start']
    rmap['L'] = rmap['L'].astype(int)



    for c in chr:
        rmap_filt = rmap[rmap['chrom'] == c]
        r_val_full = []
        chrom = []
        start_full = []
        ...
        r_val = rmap_filt['recombRate'].to_list()
        start = rmap_filt['start'].to_list()
        length_val = rmap_filt['L'].to_list()
        #rmap['I'] = rmap.index
