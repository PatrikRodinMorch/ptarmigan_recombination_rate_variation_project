...
#Identify recombination hotspots in 1Kb windows with a 1Kb step size from recombination rate estimated by ReLERNN. A recombination hotspot is defined as 10X higher recombination rate compared to the average recombination rate in 40Kb flankin regions.

populations = ["Al", "EGr", "Ice", "Ire_Scot", "Newfound", "Norw_lago", 
               "Py", "Scot", "Sva", "Swe_lago", "Swe_muta", "WGr"]
...
for p in populations:
    ...
    for c in chr:
        ...
        chrom_r_20per = np.percentile(r_val_full_flat, 20)
        ...
        for i in range(chr_len):
            ...
            if (recomb_window >= 5*recomb_flanking_left) and (recomb_window >= 5*recomb_flanking_right):
                hot_spot_5.append("T")
            else:
                hot_spot_5.append("F")
                
            if (recomb_window <= chrom_r_20per):
                cold_spot_20per.append("T")
            else:
                cold_spot_20per.append("F")                
               
            if (start_win-40000 < 0) or (stop_win+40000 > len(r_val)):
        ...
        df = pd.DataFrame({'chrom': chrom,
                    ...
                    'hot_spot_5': hot_spot_5,
                    'cold_spot_20th_percentile': cold_spot_20per,
                    'flanking_less_40Kb': flanking_less_40Kb})
