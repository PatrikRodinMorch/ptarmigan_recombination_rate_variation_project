...
### Plot the pyrho inferred recombination map for each population.
...
for p in populations:
    ...
    for c in chr:
        ...
        rmap = pd.read_csv(path+p+"_"+c+"_recombination_map_100Kb_windows.csv", sep = ",")
    ...
    my_palette = sns.color_palette(['blue'], 29)
    
    plot = sns.relplot(data=df, x='rmap_index', y='r_val', aspect=3.7, 
                    hue='chr', palette = my_palette, legend=None, kind="line") 
    ...
    plot.savefig("/home/prm/Skrivbord/pyrho_plotting/results/"+p+"_recombination_map_100Kb.pdf")
